...
export class MetricsCalculationService implements IMetricsCalculationService {
  ...
  private async calculateSecurityMetrics(repositoryId: string) {
    try {
      ...
      const highSeverityCount = vulnerabilities.filter((v: any) => v.severity === 'high' || v.severity === 'critical').length;
      const mediumSeverityCount = vulnerabilities.filter((v: any) => v.severity === 'medium').length;
      const lowSeverityCount = vulnerabilities.filter((v: any) => v.severity === 'low').length;
  ...
  private async calculateRepositoryHealth(repositoryId: string) {
    try {
      ...
      return {
        ...
        directCommitsCount: (directCommits as any)?.count || 0,
        outdatedDependencyCount: (outdatedDependencies as any)?.count || 0,
        dependencyDriftCount: (driftDependencies as any)?.count || 0,
      };
    } catch (error) {
  ...
  private async calculateCollaborationMetrics(repositoryId: string, dateRange: { start: Date; end: Date }) {
    try {
      ...
      const avgCommitsPerWeek = weeks > 0 ? ((totalCommits as any)?.count || 0) / weeks : null;

      // Check for CODEOWNERS file (mock implementation)
      const codeOwnerFileExists = Math.random() > 0.5; // TODO: Implement actual check

      return {
        activeContributorCount: (activeContributors as any)?.count || 0,
        avgCommitsPerWeek,
        codeOwnerFileExists,
      };
    } catch (error) {
